...
(*   Updated: 2025/05/13 18:42:23 by ydumaine         ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)
...
let generate_base_triplets (rna : rna) =
  let rec aux acc rna =
    ...
    | first :: second :: third :: rest ->
        aux ((first, second, third) :: acc) rest
    | _ -> acc
  in
...
let rec string_of_protein protein : string =
  ...
  let rec create_string (acc : string) protein_list =
    match protein_list with
    | head :: tail ->
        let rec aux (acc : string) protein_list =
          match protein_list with
          | [] -> acc
  ...
  let rec aux acc protein =
    match protein with
    | [] -> acc
    | aminoacid :: rest -> aux (string_of_aminoacid aminoacid :: acc) rest
  in
  create_string "" (aux [] protein)

let decode_arn arn : protein =
  let triplets = generate_base_triplets arn in
  let rec aux acc triplets =
    match triplets with
...
let () =
  ...
  print_string (helix_to_string helix_10);
  print_newline ();
  let rna_10 = generate_rna helix_10 in
  print_string (rna_to_string rna_10);
  print_newline ();
  let protein_10 = decode_arn rna_10 in
  print_string (string_of_protein protein_10);
  print_newline ();
  ...
  print_string (helix_to_string helix_100);
  print_newline ();
  let rna_100 = generate_rna helix_100 in
  print_string (rna_to_string rna_100);
  print_newline ();
  let protein_100 = decode_arn rna_100 in
  print_string (string_of_protein protein_100);
  print_newline ();

  (* Test with an empty list *)
  print_newline ();
  print_string "===== Test with an empty list =====\n";
  print_string (string_of_protein []);
  print_newline ();

  (* Test with a predefined list of amino acids *)
  print_newline ();
  print_string "===== Test with a predefined protein =====\n";
  print_string (string_of_protein [ Ala; Thr; Ser; Val; Met; Lys ]);
  print_newline ();

  (* Special test: DNA containing only T *)
  print_newline ();
  print_string
    "===== Test with a helix containing only A (should give U in RNA) =====\n";
  let helix_only_t = List.init 10 (fun _ -> ("phosphate", "deoxyribose", A)) in
  print_string (helix_to_string helix_only_t);
  print_newline ();
  let rna_only_t = generate_rna helix_only_t in
  print_string (rna_to_string rna_only_t);
  print_newline ();

  (* Special test: Check a STOP codon *)
  print_newline ();
  print_string
    "===== Test with an RNA containing a STOP codon at the start =====\n";
  let rna_stop = [ U; A; G; C; A; U; A; A; C; G; A; C ] in
  print_string (rna_to_string rna_stop);
  print_newline ();
  let stop_protein = decode_arn rna_stop in
  print_string (string_of_protein stop_protein);
  print_newline ();

  (* Special test: Check a STOP codon inside rna *)
  print_newline ();
  print_string
    "===== Test with an RNA containing a STOP codon at the middle of it =====\n";
  let rna_stop =
    [ C; C; A; A; A; G; G; A; C; U; A; G; C; A; U; A; A; C; G; A; C ]
  in
  print_string (rna_to_string rna_stop);
  print_newline ();
  let stop_protein = decode_arn rna_stop in
