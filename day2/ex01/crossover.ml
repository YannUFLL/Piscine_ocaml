...
(*   Updated: 2025/05/13 18:42:03 by ydumaine         ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)

let crossover l1 l2 =
  let rec is_in_list value list =
    match list with
    | [] -> false
  ...
  let rec create_new_list acc a =
    ...
    | [] -> acc
    | head :: tail ->
        if (not (is_in_list head acc)) && is_in_list head l2 then
          create_new_list (head :: acc) tail
        else create_new_list acc tail
  in
  create_new_list [] l1
