...
(*   Updated: 2025/05/13 18:42:26 by ydumaine         ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)
...
let generate_helix_str (str : string) =
  if String.length str <= 0 then []
    ...
    let rec aux acc i =
      ...
      else aux (generate_nucleotide (String.get str i) :: acc) (i + 1)
    in
    reverse_list (aux [] 0)
...
let rec triplets_to_string
    (triplets : (nucleobase * nucleobase * nucleobase) list) : string =
  let convert_triplet (a, b, c) : string =
    let convert_nucleobase n =
      ...
      | None -> "X"
    in
    "(" ^ convert_nucleobase a ^ ", " ^ convert_nucleobase b ^ ", "
    ^ convert_nucleobase c ^ ")"
  in
