...
let select_type list_nei k =
  ...
  for i = 0 to k - 1 do 
    ...
    let exists = 
      try 
      let _ = List.find (fun (l, _, _) -> l = label) !frequency in true with
      Not_found -> false  in
    if exists then
      frequency := List.map (fun (l, count, total_value) -> if l = label then (l, (count + 1),
        total_value +. d) else (l, count, total_value)) !frequency 
    else 
      frequency := (label, 1, d) :: !frequency
    done; 
    let best_label, _, _ = 
    List.fold_left (fun (curr_label, curr_count, curr_d) (l, count, d) -> 
      if (count > curr_count) || (curr_count = count && d < curr_d) 
        then (l, count, d) else
          (curr_label, curr_count, curr_d)) ("", 0, max_float) !frequency in
    best_label
...
let k_nn radar_list k query_radar = 
  ...
  for i = 0 to List.length radar_list - 1 do
    ...
    for j = 0 to (k - 1) do
      ...
      if (j >= !array_filling  || current_dist < sorted_dist && !array_filling = k)
        then begin 
          if (!array_filling != k) then 
            incr array_filling;
 ...
 let () =
    ...
    Printf.printf "k = %d accuracy: %.2f%%\n" i ((float_of_int !correct /. float_of_int total) *. 100.0);
      done;