...
(*   Updated: 2025/03/10 18:15:31 by ydumaine         ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)
...
let print_result expected result =
  Printf.printf "Result: %.3f | Expected: %.3f %s\n"
    result expected (if abs_float (result -. expected) < 1e-3 then "✅" else "❌")
...
let () =
  ...
  print_result 0.0 (eu_dist [|10.0; 20.0; 30.0; 40.0|] [|10.0; 20.0; 30.0; 40.0|]);
