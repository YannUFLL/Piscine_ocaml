...
(*   Updated: 2025/06/03 11:43:25 by ydumaine         ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)

class alkane_combustion ?(reactants : (Molecule.molecule * int) list = [])
  ...
  object (self)
    ...
    method get_start =
      if _mcb = [] then failwith "Reaction is not balanced" else _mca

    method get_result =
      if _mcb = [] then failwith "Reaction is not balanced" else _mcb
    ...
    method get_incomplete_results =
      ...
      let generate_all_combinaison o =
        ...
        let check_triplet (co2, co, c) =
          if (co2 * 2) + co <> o_remaining then false else true
        in
        ...
        List.map
          (fun (co2, co, c) ->
            ( o,
              (if co2 <> 0 then
                 [ ((new Molecule.carbon_dioxide :> Molecule.molecule), co2) ]
               else [])
              @ (if co <> 0 then
    ...
    method balance =
      ...
      in
      let new_reactants =
        (List.fold_right add_to_list _a_list []
          :> (Molecule.molecule * int) list)
        @ [ (new Molecule.dioxygen, scale * o_needed / 2) ]
      in

      let new_products =
        [
          (new Molecule.carbon_dioxide, c_total * scale);
          (new Molecule.water, h_total / 2 * scale);
        ]
      in
      (new alkane_combustion
         ~reactants:new_reactants ~products:new_products a_list
        :> Reaction.reaction)
