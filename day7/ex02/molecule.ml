...
class virtual molecule name (atoms: Atom.atom list) = 
  object (self)
    ...
    val _formula : string = 
      let rec add_to_list (a: Atom.atom) mol = match mol with
      | (x, q) :: tail -> if x = a#symbol then (x, q + 1) :: tail
                          else if a#symbol < x || a#symbol = "C" || (a#symbol = "H" && a#symbol != "C") 
                            then (a#symbol, 1) :: (x, q) :: tail 
                          else (x, q) :: add_to_list a tail 
      | [] -> (a#symbol, 1) :: [] in 
      let rec generate_string mol = match mol with
      | (a, q) :: tail ->  a ^ (if q > 1 then string_of_int q else "") ^ generate_string tail
      | [] -> "" in
...
class ethanol = 
  object 
    inherit molecule "Ethanol" [new Atom.carbon; 
                                new Atom.carbon; 
                                new Atom.hydrogen; 
                                new Atom.hydrogen; 
                                new Atom.hydrogen;  
...
class glucose = 
  object 
    inherit molecule "Glucose" [new Atom.carbon; 
                                new Atom.carbon; 
                                new Atom.carbon; 
                                new Atom.carbon; 
                                new Atom.carbon; 
