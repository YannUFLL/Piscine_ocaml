...
(*   Updated: 2025/01/10 09:33:02 by root             ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)

let ft_print_comb () = 
  let rec increment_units x y z =
    if (z <= 9) then (print_char(char_of_int (x + 48)); print_char(char_of_int (y + 48)); print_char(char_of_int (z + 48)); if not ((x = 7) && (y = 8) && (z = 9)) then (print_char (','); print_char (' ')); increment_units x y (z + 1))
    else () in 
