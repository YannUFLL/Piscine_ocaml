...
(*   Updated: 2025/05/31 19:55:28 by ydumaine         ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)


let safe_div x y = if y = 0 then  Try.Try.Failure (Failure "Can't divide by 0") else 
  Try.Try.return (x / y)
...
Printf.printf "Return test: %s\n" (match Try.Try.return 42 with  
 | Success x -> string_of_int x | Failure _ -> "Fail" );
...
Printf.printf "Recover test: %s\n" (match Try.Try.recover b (fun err -> match err with 
| Failure msg -> if msg = "Initial failure" then Try.Try.return 42 else Try.Try.return 0 
| _ -> Try.Try.return 84) with  
 | Success x -> string_of_int x | Failure _ -> "Fail" ); 
 
 Printf.printf "Recover test: %s\n" (match Try.Try.recover a (fun err -> match err with 
| Failure msg -> if msg = "Initial failure" then Try.Try.return 42 else Try.Try.return 0 
| _ -> Try.Try.return 84) with  
 | Success x -> string_of_int x | Failure _ -> "Fail" ); 
 ...
 | Success x -> string_of_int x | Failure _ -> "Fail" ); 
  ...
  Printf.printf "Filter test: %s\n" (match Try.Try.filter (Try.Try.return 17) (fun x -> if x mod 2 = 0 then false else true) with  
 | Success x -> string_of_int x | Failure _ -> "Fail" ); 
...
let result =
  Try.Try.return 100
  |> (fun m -> Try.Try.bind m (fun x -> safe_div x 2))
